-- Migrate the platform onto the Fintech Primitives object model.
--
-- FP becomes the system of record for investors, KYC, folios, orders,
-- mandates and payments; the tables here are our own accounts plus a local
-- mirror of FP's objects. The previous self-contained model set (funds,
-- holdings, transactions, sip_mandates, kyc_profiles, amc_houses) is replaced,
-- not migrated: its rows described a catalogue and a ledger we no longer own.
--
-- The three DELETEs below are what make the ALTERs downstream executable.
-- They removed only `prisma db seed` output -- an invented fund catalogue with
-- its NAV walk, and the dev login rows -- which `bun run db:seed` recreates
-- from the real FP sandbox catalogue snapshot. There was no investor,
-- order, folio or payment data in this database when this migration was
-- written (holdings, transactions and sip_mandates were all empty).

-- nav_history and watchlist_items are re-keyed from the dropped `funds` table
-- to `mf_schemes`, so their old rows have no scheme to point at.
DELETE FROM "nav_history";
DELETE FROM "watchlist_items";

-- `phone` becomes the required login credential. The one seeded account
-- without a phone number is recreated by the seed with one.
...
-- CreateEnum
CREATE TYPE "order_gateway" AS ENUM ('rta', 'ondc');
...
-- DropForeignKey
ALTER TABLE "funds" DROP CONSTRAINT "funds_amcId_fkey";

-- DropForeignKey
ALTER TABLE "holdings" DROP CONSTRAINT "holdings_fundId_fkey";

-- DropForeignKey
ALTER TABLE "holdings" DROP CONSTRAINT "holdings_userId_fkey";

-- DropForeignKey
ALTER TABLE "kyc_profiles" DROP CONSTRAINT "kyc_profiles_userId_fkey";

-- DropForeignKey
ALTER TABLE "nav_history" DROP CONSTRAINT "nav_history_fundId_fkey";

-- DropForeignKey
ALTER TABLE "sip_mandates" DROP CONSTRAINT "sip_mandates_fundId_fkey";

-- DropForeignKey
ALTER TABLE "sip_mandates" DROP CONSTRAINT "sip_mandates_userId_fkey";

-- DropForeignKey
ALTER TABLE "transactions" DROP CONSTRAINT "transactions_fundId_fkey";

-- DropForeignKey
ALTER TABLE "transactions" DROP CONSTRAINT "transactions_sipMandateId_fkey";

-- DropForeignKey
ALTER TABLE "transactions" DROP CONSTRAINT "transactions_userId_fkey";

-- DropForeignKey
ALTER TABLE "watchlist_items" DROP CONSTRAINT "watchlist_items_fundId_fkey";

-- DropIndex
DROP INDEX "nav_history_fundId_navDate_key";

-- DropIndex
DROP INDEX "watchlist_items_userId_fundId_key";

-- AlterTable
ALTER TABLE "audit_logs" ADD COLUMN     "actorRole" "UserRole";

-- AlterTable
ALTER TABLE "nav_history" DROP COLUMN "fundId",
ADD COLUMN     "schemeId" UUID NOT NULL;
...
-- AlterTable
ALTER TABLE "watchlist_items" DROP COLUMN "fundId",
ADD COLUMN     "schemeId" UUID NOT NULL;

-- DropTable
DROP TABLE "amc_houses";
...
-- DropTable
DROP TABLE "kyc_profiles";

-- DropTable
DROP TABLE "sip_mandates";

-- DropTable
DROP TABLE "transactions";

-- DropEnum
DROP TYPE "FundCategory";
