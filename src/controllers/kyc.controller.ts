...
export async function updateForm(req: Request<FormParams>, res: Response) {
  const body = asBody(req.body);
  const isd = optionalString(body, "mobileIsd", { maxLength: 4 });
  const number = optionalString(body, "mobileNumber", {
    pattern: /^[0-9-]{7,20}$/,
  ...
  res.json({
    data: await kycService.updateKycForm(req.params.formId, {
      ...
      residentialStatus: optionalString(body, "residentialStatus"),
      ...
      ...(typeof body["taxResidencyOtherThanIndia"] === "boolean"
        ...
        : {}),
      ...(hasGeo ? { geolocation: { latitude, longitude } } : {}),
    }),
  });
}
