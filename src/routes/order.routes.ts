...
import { getPayment } from "../services/payment.service.ts";

export const orderRouter = Router();

orderRouter.get("/", order.listOrders);
orderRouter.get("/:orderId", order.getOrder);
orderRouter.get("/:orderId/payments", async (req, res) => {
  const links = await db.paymentPurchase.findMany({ where: { mfPurchaseId: req.params.orderId }, select: { paymentId: true }, orderBy: { createdAt: "desc" } });
  res.json({ data: await Promise.all(links.map(link => getPayment(link.paymentId))) });
});
